...
export default function Page() {
  ...
  const [chatBackend] = useChatBackend()
  ...
  // 3D-Coder: the review and terminal rails are gone, so nothing to the right of
  // the chat column claims resize-driven width anymore — the preview panel
  // manages its own width and the chat column just fills what is left.
  const desktopSessionResizeOpen = createMemo(() => false)
  ...
  return (
    <SessionRouteFrame>
      ...
      >
        ...
        {/* 3D-Coder: the chat column is a plain flex-fill next to the preview
            panel — no resize-driven fixed widths now that the rails are gone. */}
        <div
          classList={{
            "@container relative flex flex-col min-h-0 h-full flex-1 min-w-0": true,
          }}
        >
          {/* 3D-Coder: the default chat brain is Claude on the user's own
              subscription (headless Claude Code); the stock opencode chat stays
              one click away behind the backend toggle. */}
          <Show
            when={chatBackend() === "claude"}
            fallback={
              settings.general.newLayoutDesigns() ? (
          ...
          >
            <ErrorBoundary fallback={sessionErrorFallback}>
              {/* Keyed on the session id so switching sessions swaps the
                  conversation with it — remounting is what gives a new
                  session an empty chat instead of the previous one. */}
              <ClaudeChat sessionID={params.id} />
            </ErrorBoundary>
          </Show>
        ...
        {/* 3D-Coder: the stock Git-review / files-changed side panel is removed in
            both layout generations — a design workspace has no code diffs to
            review. The terminal rail is gone too: Ctrl+` and the header button
