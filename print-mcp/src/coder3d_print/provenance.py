...
This is a deliberate copy of `coder3d_medimage.provenance`, not an import: the
two servers run in separate virtualenvs and coupling them would make the print
lane fail to start whenever the medical lane's dependencies move. The format is
identical and pinned by a test on each side.
"""
import hashlib
import json
...
def append_entry(case_dir: Path, *, step: str, tool: str, params: dict,
    ...
    try:
        tool_version = pkg_version("coder3d-print")
    except Exception:
        tool_version = "dev"
    entry = {
